...
fn test_split_by_kyoku() {
    TESTDATA.iter().for_each(|TestCase { desc, data }| {
        let raw_log: RawLog =
            json::from_str(data).expect(&*format!("failed to parse tenhou log (case: {desc})"));
        let splited_raw_logs = raw_log.split_by_kyoku();

        let log = Log::try_from(raw_log.clone()).expect(&*format!("invalid log (case: {desc})"));
        let joined_kyokus: Vec<_> = splited_raw_logs
            .into_iter()
            .map(RawLog::from)
            .map(|l| Log::try_from(l).expect(&*format!("invalid log (case: {desc})")))
            .flat_map(|l| l.kyokus)
            .collect();
        let joined_logs = Log {
        ...
        let mjai_log =
            tenhou_to_mjai(&log).expect(&*format!("failed to transform tenhou (case: {desc})",));
        let mjai_log_joined = tenhou_to_mjai(&joined_logs)
            .expect(&*format!("failed to transform tenhou (case: {desc})",));
